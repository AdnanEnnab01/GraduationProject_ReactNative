...
export const BuyCarHouse = () => {
  const [searchText, setSearchText] = useState('');
  const [carProducts, setCarProducts] = useState([]);
  const navigation = useNavigation();

  useEffect(() => {
    handleGetProducts(); // Fetch products when component mounts
  }, []);

  const handleGetProducts = () => {
    fetch('http://192.168.1.56:8800/api/product/car')
  ...
  const handleSearch = () => {
    ...
    if (!searchText.trim()) {
      ...
      fetch(`http://192.168.1.56:8800/api/product/car/${searchText}`)
        .then((response) => response.json())
        .then((data) => {
          setCarProducts(data);
        })
